...
const erase = async (collectionName, documentID) => {
  try {
...
const removeStaff = async (collectionName, documentID , staff) => {
  try {
    const ref = collection(collectionName).collection(staff).doc(documentID);
    const result = await ref.deleteDoc();
  } catch (error) {
    return error;
  }
};
