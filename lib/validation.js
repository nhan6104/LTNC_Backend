...
class DoctorValidation {
  ...
  validationUpdateStaff = (data) => {
    const schema = Joi.object({
      ...
      gender: Joi.string(), // Giới tính female hoặc male
      birthday: Joi.string().regex(new RegExp(process.env.REGEX_BIRTHDAY)), // Kiểm tra ngày sinh theo định dạng yyyy-mm-dd và bắt buộc
      address: Joi.string(),
      degree : Joi.string(),
